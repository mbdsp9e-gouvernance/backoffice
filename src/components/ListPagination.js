import React from "react";
import {
  CCard,
  CCardBody,
  CCardHeader,
  CCol,
  CRow,
  CTable,
  CTableBody,
  CButton,
  CTableDataCell,
  CTableHead,
  CTableHeaderCell,
  CTableRow,
  CModal,
  CModalHeader,
  CModalTitle,
  CModalBody,
  CModalFooter,
  CPaginationItem,
  CPagination,
} from "@coreui/react";
import CIcon from "@coreui/icons-react";
import { cilTrash, cilPencil, cilPlus } from "@coreui/icons";
import api from "../const/api";
import Loading from "../views/pages/Loading";
import PropTypes from "prop-types";

export default class ListPagination extends React.Component {
  static propType = {
    entity: PropTypes.string,
    urlCreate: PropTypes.string,
    urlRead: PropTypes.string,
    urlUpdate: PropTypes.string,
    urlDelete: PropTypes.string,
    tableHeader: PropTypes.array,
    tableData: PropTypes.array,
  };
  constructor(props) {
    super(props);
    this.state = {
      formModalState: false,
      deleteModalState: false,
      loading: true,
      dataFetch: null,
      error: "",
      pagination: [],
      totalPage: 0,
      pageList: 1,
      pageNumber: 10,
      id: 0,
      offset: 0,
      limit: 0,
    };
  }

  componentDidMount() {
    this.getData(1);
  }

  pagination = (totalPages) => {
    let page = [];
    for (let i = 1; i <= totalPages; i++) {
      page.push(i);
    }
    this.setState({ pagination: page });
  };

  getData = (page, pageNumber) => {
    const url = this.props.urlRead;
    const entity = this.props.entity;
    this.setLoading(true);
    fetch(api(`${url}/${page}`)).then((res) => {
      if (res.ok) {
        return res.json().then((data) => {
          this.setState({
            dataFetch: data[entity],
            pageList: page,
            pageNumber: pageNumber,
            totalPage: data.pagination.totalPages,
          });
          this.pagination(data.pagination.totalPages);
          this.setLoading(false);
        });
      }
    });
  };

  cancelFormModal = () => {
    this.setFormModalState(false);
    this.clearFields();
    this.setError();
  };

  toDelete(index) {
    this.setId(index);
    this.setDeleteModalState(true);
  }

  delete() {
    const url = this.props.urlDelete;
    fetch(api(`${url}/${this.state.dataFetch[this.state.id].id}`), {
      method: "DELETE",
    }).then((res) => {
      if (res.ok) {
        const tmp = [...this.state.dataFetch];
        tmp.splice(this.state.id, 1);
        this.setState({
          dataFetch: tmp,
        });
        this.setDeleteModalState(false);
      }
    });
  }

  update(id) {
    const url = this.props.urlUpdate;
    this.props.history.push(`${url}` + id);
  }

  add() {
    const url = this.props.urlCreate;
    this.props.history.push(`${url}`);
  }

  render() {
    const {
      dataFetch,
      loading,
      deleteModalState,
      id,
      pageList,
      pageNumber,
      pagination,
      totalPage,
    } = this.state;
    const { entity, tableHeader, tableData } = this.props;
    console.log(tableHeader);
    if (loading) {
      return <Loading />;
    }
    return (
      <>
        <CRow>
          <CCol xs={12}>
            <CCard className="mb-4">
              <CCardHeader>
                <span className="d-grid gap-2 d-md-flex justify-content-between">
                  <strong>Liste des {entity}</strong>
                  <CButton color={"primary"} onClick={() => this.add()}>
                    <CIcon icon={cilPlus} className="me-2" />
                    Ajouter
                  </CButton>
                </span>
              </CCardHeader>
              <CCardBody>
                <CTable>
                  <CTableHead>
                    <CTableRow>
                      {tableHeader &&
                        tableHeader.map((cell, index) => (
                          <CTableHeaderCell scope="col" key={index}>
                            {cell.header}
                          </CTableHeaderCell>
                        ))}
                    </CTableRow>
                  </CTableHead>

                  <CTableBody>
                    {dataFetch &&
                      dataFetch.map((cell, index) => (
                        <CTableRow key={cell.id}>
                          <CTableDataCell>
                            {cell[tableHeader[0].header]}
                          </CTableDataCell>
                          {/* <CTableDataCell>
                            {data.price > 1000 ? (
                              <>{data.price.toLocaleString()} Ar</>
                            ) : (
                              <>{data.price} Ar</>
                            )}
                          </CTableDataCell>
                          <CTableDataCell>
                            <CButton
                              color={"light"}
                              onClick={() => this.update(data.id)}
                            >
                              <CIcon icon={cilPencil} />
                            </CButton>
                          </CTableDataCell> */}
                          <CTableDataCell>
                            <CButton
                              color={"danger"}
                              onClick={() => this.toDelete(index)}
                            >
                              <CIcon icon={cilTrash} />
                            </CButton>
                          </CTableDataCell>
                        </CTableRow>
                      ))}
                  </CTableBody>
                </CTable>
                {/* <CPagination aria-label="Page navigation example">
                  <CPaginationItem
                    aria-label="Previous"
                    disabled={pageList === 1}
                    onClick={() =>
                      this.getServiceData(pageList - 1, pageNumber)
                    }
                  >
                    <span aria-hidden="true">&laquo;</span>
                  </CPaginationItem>
                  {pagination.map((page, index) => (
                    <CPaginationItem
                      key={index}
                      active={page === pageList}
                      onClick={() => this.getServiceData(page, pageNumber)}
                    >
                      {` ${page} `}
                    </CPaginationItem>
                  ))}
                  <CPaginationItem
                    aria-label="Next"
                    disabled={
                      totalPage - pageList === 0 || services.length === 0
                    }
                    onClick={() =>
                      this.getServiceData(pageList + 1, pageNumber)
                    }
                  >
                    <span aria-hidden="true"> &raquo; </span>
                  </CPaginationItem>
                </CPagination> */}
              </CCardBody>
            </CCard>
          </CCol>
        </CRow>
        {/* <CModal
          alignment="center"
          visible={deleteModalState}
          onClose={() => this.setDeleteModalState(false)}
        >
          <CModalHeader>
            <CModalTitle>Supprimer</CModalTitle>
          </CModalHeader>
          <CModalBody>
            <p>
              Voulez vous supprimer{" "}
              {services &&
                services.length > 0 &&
                id < services.length &&
                services[id].entitled}
              ?
            </p>
          </CModalBody>
          <CModalFooter>
            <CButton
              color="secondary"
              onClick={() => this.setDeleteModalState(false)}
            >
              Annuler
            </CButton>
            <CButton color="danger" onClick={() => this.delete()}>
              Supprimer
            </CButton>
          </CModalFooter>
        </CModal> */}
      </>
    );
  }
  setOffset = (o) => {
    this.setState({
      offset: o,
    });
  };
  setLimit = (l) => {
    this.setState({
      limit: l,
    });
  };
  setLoading(state) {
    this.setState({
      loading: state,
    });
  }
  setId(id) {
    this.setState({
      id: id,
    });
  }
  setDeleteModalState(modal) {
    this.setState({
      deleteModalState: modal,
    });
  }
}
